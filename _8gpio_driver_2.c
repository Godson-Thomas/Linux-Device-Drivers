...
MODULE_DESCRIPTION("A simple gpio driver for setting 2 LED's and reading a button");
...
#define DRIVER_NAME "gdsn_gpio_driver"
...
static int __init ModuleInit(void) {
	printk("Hello, Kernel- _8gpio_driver_2.ko!\n");
	...
	if(gpio_request(26, "rpi-gpio-26")) {
		printk("Cannot allocate GPIO 26\n");
		goto Gpio26Error;
	}
	
		/* Set GPIO 26 direction */
	if(gpio_direction_output(26, 0)) {
		printk("Cannot set GPIO 26 to output!\n");
		goto Gpio26Error;
	}
...
static void __exit ModuleExit(void) {
	...
	class_destroy(my_class);
	unregister_chrdev_region(my_device_nr, 1);
	printk("Goodbye, Kernel: _8gpio_driver_2.ko\n");
}
